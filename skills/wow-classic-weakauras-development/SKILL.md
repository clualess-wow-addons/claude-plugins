...
**Core principles:**
1. **Built-in triggers first.** Two popular classic debuff trackers we decoded contain zero custom Lua, and the big class-pack families (e.g. Cludes) are described as built-in-only. Reach for custom code only when no built-in trigger covers it; built-in trigger configuration is UI-discoverable in `/wa` — this skill documents the custom-code layer.
